...
function Item({ title = "", datas }) {
  ...
  useEffect(() => {
    datas.map((_, index) => {
      document.querySelector(`[data-index="${title}${index}"]`).innerHTML =
        datas[index];
    });
  }, [datas]);
  return (
    <details open>
      <summary
        style={{
          cursor: "pointer",
          color: datas.length === 0 ? "crimson" : null,
        }}
      >
        {title}
      </summary>
...
const publish_time_selectors = [
  "meta[property='article:published_time']",
  "meta[name='pubdate']",
  "meta[name='its_publication']",
];
const MetaTag = memo(function () {
  const [isShowMetaTags, setIsShowMetaTags] = useState(false);
  const [typeTag, setTypeTag] = useState(null);
  const [publishTimeTags, setPublishTimeTags] = useState([]);
  const getMetaTags = useCallback(() => {
    setTypeTag($("meta[property='og:type']")?.outerHTML);
    const _publish_time_tags = publish_time_selectors.map(
      (selector) => $(selector)?.outerHTML
    );
    setPublishTimeTags(_publish_time_tags.filter((tag) => tag));
  }, []);
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        setIsShowMetaTags(true);
        getMetaTags();
      }}
      style={{ border: "1px solid #3f3f3f", borderRadius: 4, padding: 8 }}
    >
      {!isShowMetaTags ? (
        <Button>Get meta tags</Button>
      ) : (
        <>
          <details open>
            <summary style={{ cursor: "pointer" }}>Meta type</summary>
            <p
              style={{
                paddingLeft: 16,
                color: typeTag ? "var(--pink)" : "crimson",
              }}
            >
              {typeTag ?? "null"}
            </p>
          ...
          <details open>
            <summary style={{ cursor: "pointer" }}>Meta Publish date</summary>
            <ul style={{ paddingLeft: 16 }}>
              {publishTimeTags.length > 0 ? (
                publishTimeTags.map((tag, i) => (
                  <li
                    key={i}
                    style={{
                      color: tag ? "var(--pink)" : "crimson",
                      marginLeft: 8,
                    }}
                  >
                    {tag}
  ...
  const handleSubmit = useCallback((value, type) => {
    ...
    try {
      ...
      for (let key in jsonObj) {
        ...
        data[key] = Array.from(document.querySelectorAll(jsonObj[key])).map(
    ...
    <div className="Container">
      <main>
        ...
        >
          ...
          <ClearFloat>
            <small style={{ color: "var(--blue)", float: "right" }}>
              {"tips >> SHIFT ENTER to submit"}
            </small>
          </ClearFloat>
        </form>
        <p
          style={{
            color: isSubmitted ? "#00a44d" : "var(--yellow)",
            fontWeight: "bolder",
          }}
        >{`${isSubmitted ? "RESULT:" : "Not submitted..."}`}</p>
